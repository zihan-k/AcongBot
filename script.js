    // Kode Google API
const GOOGLE_API_KEY = "AIzaSyAs0DRkqcDtXA_FPnUoF46z5jzp9Bo7eOo";
...
const translations = {
  id: {
    ...
    "aboutDeveloper": "Tentang Developer",
    "searchGoogle": "Cari",
    // Features
...
function updateCustomInstructions() {
  const newInstructions = prompt("Masukkan instruksi untuk AcongBot", customInstructions);
...
function setLanguage(lang) {
  ...
  document.querySelector('.AboutDev').innerHTML = `${t.aboutDeveloper} <i class="fa-solid fa-up-right-from-square"></i>`;
  ...
  document.querySelector('.cari-button').innerHTML = `<i class="fas fa-globe"></i> ${t.searchGoogle}`;
  document.querySelector('.file-upload-option').innerHTML = `<i class="fas fa-image"></i> ${t.uploadImage}`;
  document.querySelector('.file-upload-option1').innerHTML = `<i class="fas fa-file"></i> ${t.uploadFile}`;
...
// Tambahkan fungsi-fungsi ini di script.js

// Toggle dropdown unggah file
function toggleFileUpload() {
  const dropdown = document.getElementById('fileUploadDropdown');
...
document.addEventListener('click', function(event) {
  ...
  if (!dropdown.contains(event.target) && !attachButton.contains(event.target)) {
...
function searchGoogle() {
    ...
    if (input) {
      // Tambahkan ke chat (jika ingin tampil juga)
      addBubble(`Mencari di Google: ${input}`, 'user');
...
function startSpeechRecognition() {
  ...
  if ('webkitSpeechRecognition' in window || 'SpeechRecognition' in window) {
    ...
    recognition.lang = 'id-ID'; // Sesuaikan dengan bahasa yang dipilih
    recognition.interimResults = false;
    
    // Ubah ikon mikrofon saat mulai mendengarkan
    document.getElementById('voiceButton').innerHTML = '<i class="fas fa-microphone" style="color: #ff6b6b;"></i>';
...
function handleImageUpload(file) {
  ...
  reader.onload = function(e) {
    ...
    // Tambahkan pesan dengan gambar ke chat
    const message = document.createElement('div');
    message.className = 'bubble user';
    message.innerHTML = `<img src="${imageData}" alt="Uploaded Image" style="max-width: 100%; max-height: 300px; border-radius: 8px;">`;
    
    document.getElementById('chatBox').appendChild(message);
    document.getElementById('chatBox').scrollTop = document.getElementById('chatBox').scrollHeight;
    
    // Sembunyikan dropdown
    document.getElementById('fileUploadDropdown').classList.remove('show');
    
    // Di sini Anda bisa menambahkan kode untuk mengirim gambar ke backend
  };
...
function handleFileUpload(file) {
  // Tambahkan pesan dengan file ke chat
  const message = document.createElement('div');
  message.className = 'bubble user';
  message.innerHTML = `
  ...
  // Di sini Anda bisa menambahkan kode untuk mengirim file ke backend
}
